# MedRAX 專案憲法 (Project Constitution)

> 🤖 **MedRAX**: Medical Reasoning Agent for Chest X-ray
>
> 醫學影像 AI 代理 - 整合多模態模型與專業工具的通用框架
...
2. 醫學推理 (Medical Reasoning) 為核心領域
3. 工具整合 (Tool Integration) 與模型封裝 (Model Wrapping) 為基礎設施
4. 使用醫學統一語言（Ubiquitous Language in Medical Domain）

### 第 2 條：DAL 資料存取層獨立
1. Data Access Layer 必須獨立於業務邏輯
2. Repository Pattern 為唯一資料存取方式
3. 禁止在 Medical Reasoning Layer 直接操作 DICOM/外部服務
4. 所有醫學工具 (CheXagent, LLaVA-Med, MedSAM 等) 必須透過 Service Layer

### 第 3 條：分層架構
```
├── Domain/          # 醫學推理核心（diagnosis logic, reasoning）
├── Application/     # 應用層（agent orchestration、query handling）
├── Infrastructure/  # 基礎設施（tool wrappers、DAL、DICOM processing）
└── Presentation/    # 呈現層（Gradio interface、API endpoints）
```
